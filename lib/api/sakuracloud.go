...
func (c *Client) DeleteDnsRecord(zoneName string, hostName string, ip string) error {
	...
	if dnsItem.HasRecord() {
		...
		err = c.deleteDnsServiceItem(dnsItem)
		if err != nil {
			return err
		}
...
func (c *Client) getDnsCommonServiceItem(zoneName string) (*sakura.CommonServiceItem, error) {
	...
	var dnsZone sakura.SearchResponse
	if err := json.Unmarshal(data, &dnsZone); err != nil {
		return nil, err
	}

	//すでに登録されている場合
	var dnsItem *sakura.CommonServiceItem
	if dnsZone.Count > 0 {
		dnsItem = &dnsZone.CommonServiceItems[0]
	} else {
		dnsItem = sakura.CreateNewCommonServiceItem(zoneName)
	}

	return dnsItem, nil
}

func (c *Client) updateDnsRecord(dnsItem *sakura.CommonServiceItem) (*sakura.CommonServiceItem, error) {
...
func (c *Client) deleteDnsServiceItem(dnsItem *sakura.CommonServiceItem) error {
	var (
		method string
		uri    string
	)
	...
	_, err := c.newRequest(method, uri, dnsItem)
	if err != nil {
		return err
	}
